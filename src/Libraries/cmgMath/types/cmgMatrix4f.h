...
struct Matrix4f
{
	...
	explicit Matrix4f(const Vector3f& xBasis,
		const Vector3f& yBasis, const Vector3f& zBasis);
	explicit Matrix4f(const Vector3f& xBasis, const Vector3f& yBasis,
		const Vector3f& zBasis, const Vector3f& translation);
	explicit Matrix4f(const Vector4f& xColumn, const Vector4f& yColumn,
	...
	Vector3f TransformAffine(const Vector3f& vec) const;
	Vector3f TransformAffineInverse(const Vector3f& vec) const;
	Vector3f Rotate(const Vector3f& vec) const;
	Vector3f RotateInverse(const Vector3f& vec) const;
	...
	Matrix4f& InitPerspectiveXY(float fovX, float fovY,
		float zNear, float zFar);
	Matrix4f& InitInversePerspectiveX(float fovX, float aspect,
		float zNear, float zFar);
	Matrix4f& InitInversePerspectiveY(float fovY, float aspect,
		float zNear, float zFar);
	Matrix4f& InitInversePerspectiveXY(float fovX, float fovY,
		float zNear, float zFar);
	...
	static Matrix4f	CreateLookAt(const Vector3f& lookAtDir, const Vector3f& upDir);
	...
	static Matrix4f	CreatePerspectiveXY(float fovX, float fovY,
		float zNear, float zFar);
	static Matrix4f	CreateInversePerspective(float fovY, float aspect,
		float zNear, float zFar);
	static Matrix4f	CreateInversePerspectiveX(float fovX, float aspect,
		float zNear, float zFar);
	static Matrix4f	CreateInversePerspectiveXY(float fovX, float fovY,
		float zNear, float zFar);
	static Matrix4f	CreateOrthographic(float left, float right,
		float bottom, float top, float znear, float zfar);
	
	// Static matrix operations
	static void Multiply(const Matrix4f& left,
		const Matrix4f& right, Matrix4f& outMat);
	static Matrix4f Multiply(const Matrix4f& left, const Matrix4f& right);
	static void Lerp(const Matrix4f& a, const Matrix4f& b,
		float t, Matrix4f& outMat);
...
private:
	...
	void DoInitInversePerspective(float x1, float y2, float zNear, float zFar);
};


#endif // _CMG_MATRIX_4F_H_