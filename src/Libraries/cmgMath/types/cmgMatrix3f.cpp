...
const Matrix3f Matrix3f::IDENTITY(1,0,0, 0,1,0, 0,0,1);
...
// Set the individual columns.
void Matrix3f::Set(	const Vector3f& xColumn,
...
void Matrix3f::Invert(const Matrix3f& src, Matrix3f& dst)
{
	...
	CMG_ASSERT_MSG(det != 0.0f, "3x3 matrix is invertible (determinant is zero)");
...
Vector3f Matrix3f::Transform(const Vector3f& vec) const
{
	Vector3f result;
	result.x = (m[0] * vec.x) + (m[3] * vec.y) + (m[6] * vec.z);
	result.y = (m[1] * vec.x) + (m[4] * vec.y) + (m[7] * vec.z);
	result.z = (m[2] * vec.x) + (m[5] * vec.y) + (m[8] * vec.z);
	return result;
}

Vector3f Matrix3f::TransformTranspose(const Vector3f& vec) const
{
	Vector3f result;
...
Matrix3f& Matrix3f::InitSkewSymmetric(const Vector3f& v)
{
	m[0] = 0.0f; m[3] = -v.z; m[6] = v.y;
	m[1] = v.z;  m[4] = 0.0f; m[7] = -v.x;
	m[2] = -v.y; m[5] =  v.x; m[8] = 0.0f;
	return *this;
}
...
Matrix3f Matrix3f::CreateSkewSymmetric(const Vector3f& v)
{
	Matrix3f result;
